...
function gridColumns (node: Node) {
    ...
    const lineHeight = parseFloat(window.getComputedStyle(node as HTMLElement).getPropertyValue("--line-height")) * 16; // rem to px
    const gridColumnWidth = (containerWidth - lineHeight / 2 * 4) / 5;
    (node as HTMLElement).style.gridTemplateColumns = `repeat(5, minmax(${gridColumnWidth}px, 1fr))`;
}
...
function gridSpan (node: Node) {
    ...
    const itemWidth = context.measureText((node as HTMLElement).textContent!).width + 16 + 7; // Fucking margin + marker size
