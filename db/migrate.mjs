...
function printUsageAndExit() {
	console.error(`Usage: \x1b[4m${process.argv[1]}\x1b[m <database> [<start_migration>]`);
	process.exit(1);
}
...
if (process.argv.length === 4 && !await access(database, constants.R_OK | constants.W_OK).catch(() => false)) {
...
const appliedMigrationsResult = await sqliteClient.execute('SELECT "id" FROM "_migrations" WHERE "applied" = 1');
